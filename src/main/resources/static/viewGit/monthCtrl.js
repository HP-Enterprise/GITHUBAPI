define(['../scripts/git', 'jquery'], function (module, $) {
    module.controller("monthCtrl", function ($scope, $http) {
        ...
        $scope.getAllGitHubWork = function (flag) {
            ...
            $http.get("/api/workOfMonth", $scope.workSearch).success(function (data, status, headers) {
        ...
        $scope.DownloadExcel = function (flag) {
            ...
            $scope.url = "http://localhost:8080/api/exportMonthExcel?" + $scope.REALNAME + "&" + $scope.USERNAME + "&" + $scope.WEEKNUM+"&"+$scope.YEAR;
