define(['../scripts/git', 'jquery'], function (module, $) {
    module.controller("quarterCtrl", function ($scope, $http) {
        ...
        $scope.getAllGitHubWork = function (flag) {
            ...
            $http.get("/api/workOfQuarter", $scope.workSearch).success(function (data, status, headers) {
        ...
        $scope.DownloadExcel = function (flag) {
            ...
            $scope.url = "http://localhost:8080/api/exportQuarterExcel?" + $scope.REALNAME + "&" + $scope.USERNAME + "&" + $scope.WEEKNUM+"&"+$scope.YEAR;
